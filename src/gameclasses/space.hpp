...
public:
  ...
  void NewLvlPrepare(int const lvl)
  {
    ...
    ALIEN_SHOOT_SPEED = ALIEN_SHOOT_SPEED_DEFAULT - (lvl * ALIEN_SHOOT_SPEED_INC);
    if (ALIEN_SHOOT_SPEED < ALIEN_SHOOT_SPEED_MIN) ALIEN_SHOOT_SPEED = ALIEN_SHOOT_SPEED_MIN;
    if (lvl / ALIEN_HEALTH_LVL > 0) ALIEN_HEALTH = ALIEN_HEALTH_DEFAULT + (lvl / ALIEN_HEALTH_LVL * ALIEN_HEALTH_INC);
    if (ALIEN_HEALTH > ALIEN_HEALTH_MAX) ALIEN_HEALTH = ALIEN_HEALTH_MAX;
