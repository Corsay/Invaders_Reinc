...
TEST(gun2d_test, test_construction)
{
  ...
  EXPECT_EQ(g1.GetSpeedShoot(), GUN_SPEED_SHOOT_START);
  ...
  EXPECT_EQ(g2.GetSpeedShoot(), GUN_SPEED_SHOOT_START);
  EXPECT_EQ(g2.GetLives(), GUN_LIVES_START);
  EXPECT_EQ(g2.GetRate(), 0);

  Gun2D g3 = { {5.2f, 8.4f}, {3.4f, 7.8f}, 5, 55, 90 };
  EXPECT_EQ(g3.GetHealth(), 55);
  EXPECT_EQ(g3.GetSpeedShoot(), 90);
...
TEST(gun2d_test, test_assignment)
{
  ...
  EXPECT_EQ(g1, Gun2D (Point2D(1.2f, 2.4f), Point2D(3.6f, 4.8f), GUN_LIVES_START, GUN_HEALTH_START, GUN_SPEED_SHOOT_START));
